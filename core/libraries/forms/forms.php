...
class forms {
	...
	public static function open($action = NULL, $attr = array(), $hidden = NULL) {
		...
		// Form opening tag
		$form = '<form' . forms::attributes($attr) . '>' . "\n";

		// Add hidden fields immediate after opening tag
		empty($hidden) or $form .= forms::hidden($hidden);
	...
	public static function open_multipart($action = NULL, $attr = array(), $hidden = array()) {
		...
		return forms::open($action, $attr, $hidden);
	}
	...
	public static function legend($text = '', $data = NULL, $extra = '') {
		return '<legend' . forms::attributes((array) $data) . ' ' . $extra . '>' . $text . '</legend>' . "\n";
	}
	...
	public static function input($data, $value = '', $extra = '') {
		...
		return '<input' . forms::attributes($data) . ' ' . $extra . ' />';
	}
	...
	public static function password($data, $value = '', $extra = '') {
		...
		return forms::input($data, $value, $extra);
	}
	...
	public static function dropdown($data, $options = NULL, $selected = NULL, $extra = '') {
		...
		$input = '<select' . forms::attributes($data, 'select') . ' ' . $extra . '>' . "\n";
	 ...
	 * @return  string
	 */
	public static function dropdown_simple($data, $options = NULL, $selected = NULL, $extra = '', $null_first = null) {
		if (!is_array($data)) {
			$data = array('name' => $data);
		} else {
			if (isset($data['options'])) {
		...
		$input = '<select' . forms::attributes($data, 'select') . ' ' . $extra . '>' . "\n";
		if ($null_first) {
			$input .= '<option value="0">' . $null_first . '</option>' . "\n";
		}
		foreach ((array) $options as $key => $val) {
			// Key should always be a string
			$key = (string) $key;
	...
	public static function genderSelectList($name, $selected) {
		return self::dropdown_simple($name, array('no_gender' => _GENDER_NONE, 'male' => _MALE, 'female' => _FEMALE), $selected);
	}
	...
	public static function button($data = '', $value = '', $extra = '') {
		...
		return '<button' . forms::attributes($data, 'button') . ' ' . $extra . '>' . $value . '</button>';
	}
