...
class joosDatabase {
	...
	protected function __construct($host = 'localhost', $user = 'root', $pass = '', $db = '', $debug = 0, $port = null, $socket = null) {
		try {
			$dns = sprintf('mysql:host=%s;dbname=%s', $host, $db);
			self::$_instance = new PDO($dns,
							$user,
							$pass,
							self::$opts);
	 ...
	 */
	protected function old__construct($host = 'localhost', $user = 'root', $pass = '', $db = '', $debug = 0, $port = null, $socket = null) {
		$this->_debug = $debug;
...
// TODO исправить, метод CLONE используется при кешированиии и сериалзации модели
	...
	public function query() {
		...
		$this->_error_num = 0;
		$this->_error_msg = '';
		$this->_cursor = self::$_instance->query($this->_sql);
...
class joosDatabaseUtils extends joosDatabase {
	...
	public function query_batch($abort_on_error = true, $transaction_safe = false) {
		...
		foreach ($query_split as $command_line) {
			...
			if ($command_line != '') {
				$this->_cursor = self::$_instance->query($this->_resource, $command_line);
				if (!$this->_cursor) {
					$error = 1;
