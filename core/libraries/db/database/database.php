...
class database{
	...
	public static function instance() {
		...
		JDEBUG ? jd_inc('database::instance()') : null;
		/*
		  if (function_exists('debug_backtrace')) {
		  $n = true;
		  foreach (debug_backtrace() as $back) {
		  if ( $n && @$back['file']) {
		  JDEBUG ? jd_inc('database::instance()->'.$back['file'].': '.$back['line']) : null;
		  $n = false;
		  }
		  }
		  $n = true;
		...
		if (self::$instance === NULL) {
			...
			$database = new database($db_config['host'], $db_config['user'], $db_config['password'], $db_config['name'], $db_config['prefix'], $db_config['debug']);
	 ...
	 * Работает ТОЛЬКО через database::instance()->insert_object
...
class joosDBModel {
	...
	function bind(array $array, $ignore = '') {
		return database::bind_array_to_object($array, $this, $ignore);
	}
