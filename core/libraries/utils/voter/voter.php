...
class joosVoter {
	 ...
	 * @return joosVoter
	 */
	public static function instance($obj_name) {
	...
	public function add_from_user($obj, $user, $vote_ball) {
		...
		$vote->user_ip = joosRequest::user_ip();
		$vote->vote = $vote_ball;
		...
		$this->message = ( $result ) ? __('Голос принят') : __('Всё плохо');
		$this->counter = $result;
	...
	public function get_count($obj) {
		$c = database::instance()->set_query('SELECT votes_count FROM #__votes_' . self::$_obj_name . '_results WHERE obj_id=' . (int) $obj->id)->load_result();
		return $c ? $c : 0;
	}

	public function get_count_by_id($id) {
		$c = database::instance()->set_query('SELECT votes_count FROM #__votes_' . self::$_obj_name . '_results WHERE obj_id=' . (int) $id)->load_result();
		return $c ? $c : 0;
	}
	...
	public static function get_count_voters($obj) {
		$c = database::instance()->set_query('SELECT count(obj_id) FROM #__votes_' . self::$_obj_name . ' WHERE obj_id=' . (int) $obj->id)->load_result();
		return $c ? $c : 0;
	}
	...
	public static function get_votes_average($obj) {
		$c = database::instance()->set_query('SELECT votes_average FROM #__votes_' . self::$_obj_name . '_results WHERE obj_id=' . (int) $obj->id)->load_result();
		return $c ? $c : 0;
	}
	...
	public static function get_count_vote_from_user_to_obj(User $user, $obj) {
		$c = database::instance()->set_query('SELECT vote FROM #__votes_' . self::$_obj_name . ' WHERE user_id=' . $user->id . ' obj_id=' . (int) $obj->id)->load_result();
		return $c ? $c : 0;
	}

	/**
	 * Вывод контроллов для голосования
	 * @param str $obj_type - тип голосовалки (принадлежность компоненту)
	 * @param integer $obj_id - id объекта рейтингования
	 * @param integer $votesresults - результаты голосования по объекту
	 */
	public static function controls($obj_type, $obj_id, $votesresults = 0) {
		$active_plus = '';
		$active_minus = '';

		if (isset(User::current()->extra()->cache_votes[$obj_type][$obj_id])) {
			if (User::current()->extra()->cache_votes[$obj_type][$obj_id] == 1) {
				$active_plus = ' active';
				$active_minus = ' unactive';
			} else {
				$active_plus = ' unactive';
				$active_minus = ' active';
			}
		}
		$votesresults = $votesresults ? $votesresults : 0;


		return '<span class="g-vote_content"><b class="rater_content g-vote_plus' . $active_plus . '" title="Нравится" obj_type="' . $obj_type . '"  obj_id="' . $obj_id . '">+</b>
				 <span class="item_rate obj_' . $obj_type . '_' . $obj_id . '">' . $votesresults . '</span>
				<b class="rater_content g-vote_minus' . $active_minus . '" title="Не нравится" obj_type="' . $obj_type . '"  obj_id="' . $obj_id . '">-</b></span>';
	}
 ...
 */
class Votes extends joosDBModel {
	...
	function __construct($obj_name) {
		$this->joosDBModel('#__votes_' . $obj_name, null); // FALSE - что бы не пытался обновлять несуществующие записи
	...
	public function check_user_vote_by_day(User $user, $obj) {
		...
		return (bool) $this->_db->set_query($sql, 0, 1)->load_result() > 0;
	}
 ...
 */
class VotesResult extends joosDBModel {
	...
	function __construct($obj_name) {
		$this->joosDBModel('#__votes_' . $obj_name . '_results', 'id');
	}
	...
	public function add($obj_id, $vote) {
		...
		return $this->_db->set_query($sql)->query() ? $this->load_counter($obj_id) : false;
	}

	function load_counter($obj_id) {
