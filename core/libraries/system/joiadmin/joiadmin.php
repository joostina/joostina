...
class JoiAdmin {
	...
	public static function dispatch() {
		...
		JDEBUG ? jd_log('joiAdmin::dispatch() - ' . $class . '::' . $task) : null;
...
// упрощенная система получения пагинатора
	 ...
	 * 						(т.е. когда метод вызывается из JoiAdmin::listing или JoiAdmin::edit)
			...
			foreach ($extra_data as $key => $value) {
				switch ($key) {
					...
					case 'search':
						...
						if ($search_value !== false && Jstring::trim($search_value) != '') {
							foreach ($value as $field_name => $selected_value) {
								$wheres_search[] = sprintf('%s LIKE ( %s )', database::instance()->name_quote($selected_value), database::instance()->quote("%" . $search_value . "%"));
							}
						}
						break;
					...
					case 'filter':
						...
						foreach ($value as $params_key => $params_value) {
							...
							if (($selected_value && $selected_value != -1) OR $selected_value === '0') {
								$wheres_filter[] = sprintf('%s=%s', database::instance()->name_quote($field_name), database::instance()->quote($selected_value));
							}
						}
						break;
	...
	private static function get_extrainfo(joosDBModel $obj) {
		...
		if (isset($fields_info['state'])) {
			...
			$header_extra['filter'] += array(
				'state' => array(
					'name' => 'Состояние',
					'call_from' => 'JoiAdmin::get_state_selector'
				),
			);
		}
		return $header_extra;
