...
class JooSSICache
{
    ...
    public function __construct($path = '')
    {
        if ($path == '') {
            if (isset(Doo::conf()->CACHE_PATH))
                $this->_directory = Doo::conf()->CACHE_PATH . 'block/';
            else
                $this->_directory = Doo::conf()->SITE_PATH . Doo::conf()->PROTECTED_FOLDER . 'cache/block/';
        } else {
    ...
    public function getPart($id, $secondsCache = 3600)
    {
        ...
        // If the cache has not expired, include it.
        if (file_exists($this->_cachefile) && time() - $secondsCache < filemtime($this->_cachefile)) {
            $root = strtolower(str_replace("\\", '/', JPATH_BASE));
            //$this->_cachefile = str_replace("j:/nimp/htdocs/jsound.local", '', $this->_cachefile);
            echo '<!--# include file="' . str_replace($root, '', $this->_cachefile) . '"-->';
            return true;
        }
    }
    ...
    public function flushPart($id)
    {
        $deleteNum = 0;
        if (is_string($id)) {
            if (file_exists($this->_directory . $id . '.html')) {
                unlink($this->_directory . $id . '.html');
                $deleteNum++;
            }
        } else {
            foreach ($id as $f) {
                if (file_exists($this->_directory . $f . '.html')) {
                    unlink($this->_directory . $f . '.html');
                    $deleteNum++;
                }
            }
        }
