...
class actionsMedias{
    ...
    public static function save( $option, $id, $page, $task, $create_new = false ) {
		...
		$games = Media::get_games();
		$obj_data->game_name = $games[$obj_data->game_id];
		$obj_data->store();

		$create_new ? mosRedirect( 'index2.php?option='.$option.'&task=create', 'Сохранено успешно!, Создаём новое' ) : mosRedirect( 'index2.php?option='.$option , 'Сохранено успешно!');
	}
