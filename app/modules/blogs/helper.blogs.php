...
class modulesHelperBlogs {
	...
	public static function get_latest( $limit = 10 ) {
		...
		return $blogs->get_list( array ( 'select' => "b.id, b.title, u.id as userid, b.created_at, u.username,c.slug as cat_slug" ,
		                                 'join'   => 'as b INNER JOIN #__blog_category AS c ON ( c.id=b.category_id AND c.state=1 )' . ' INNER JOIN #__users AS u ON (u.id=b.user_id AND u.state=1) ' ,
		                                 'where'  => 'b.state=1' ,
		                                 'limit'  => $limit ,
	...
	public static function get_latest_by_cat_id( $category_id , $limit = 10 ) {
		...
		// опубликованные записи блога
		return $blogs->get_list( array ( 'select' => "b.id, b.title, u.id as userid, b.created_at, u.username,c.slug as cat_slug" ,
		                                 'join'   => 'as b INNER JOIN #__blog_category AS c ON ( c.id=b.category_id AND c.state=1 )' . ' INNER JOIN #__users AS u ON u.id=b.user_id' ,
