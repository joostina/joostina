...
class actionsNews extends joosController {
	...
	public static function action_before( $active_task ) {
		...
		//Хлебные крошки
		joosBreadcrumbs::instance()->add( 'Новости' , $active_task == 'index' ? false : joosRoute::href( 'news' ) );
	...
	public static function index() {
		...
		$news_items = $news->get_list( array ( 'select' => '*' ,
		                                       'offset' => $pager->offset ,
		                                       'limit'  => $pager->limit ,
		                                       'where'  => 'state = 1' ,
		...
		return array ( 'news_items' => $news_items ,
		               'pager'      => $pager );
	}
