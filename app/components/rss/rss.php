...
class rssStok {
	...
	function getParameters() {
		...
		if (method_exists($database, "getNullDate")) {
			$params->nullDate = $database->get_null_date();
		} else {
	...
	public static function feed($content) {
...
//		$params = rssStok::getParameters($id);
		...
		if (count($rows)) {
			foreach ($rows as $row) {
				...
				$item_description = Text::text_clean($item_description);
...
function video() {
	...
	return $database->load_object_list();
}