...
class actionsSearch extends joosController {
	...
	public static function action_before( $active_task ) {
		...
		//Хлебные крошки
		joosBreadcrumbs::instance()->add( 'Поиск' , $active_task == 'index' ? false : joosRoute::href( 'search' ) );
	...
	private static function _search_catalog( $searchword ) {
		...
		$_results = $content->get_list( array ( 'select'    => 'c.*, ef.*, ef.value AS sku' ,
		...
		$pager    = new joosPager( joosRoute::href( 'search_process' , array ( 'slug'       => 'catalog' ,
		                                                                       'searchword' => $searchword ) ) , $count , 3 , 5 );
		$pager->paginate( $page );

		$results = $content->get_list( array ( 'select'    => 'c.*, ef.*, ef.value AS sku' ,
		                                       'join'      => 'LEFT JOIN #__extrafields_data AS ef ON ( (ef.obj_id = c.id AND ef.field_id = 2))' ,
		                                       'where'     => "LOWER(c.title) LIKE LOWER('%{$searchword}%')
						OR LOWER(c.fulltext) LIKE LOWER('%{$searchword}%')
						OR LOWER(ef.value) LIKE LOWER('%{$searchword}%') " ,
	...
	private static function _search_news( $searchword ) {
		...
		$pager   = new joosPager( joosRoute::href( 'search_process' , array ( 'slug'       => 'news' ,
		                                                                      'searchword' => $searchword ) ) , $count , 3 , 5 );
		$pager->paginate( $page );

		//Ищем по основному содержимому
		$results = $content->get_list( array ( 'select' => '*' ,
		                                       'where'  => $where ,
		                                       'offset' => $pager->offset ,
		                                       'limit'  => $pager->limit ,
		                                       'order'  => 'id DESC' ) );
