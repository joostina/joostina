...
class Categories extends NestedSet {
	...
	public function before_store() {
		$this->slug = $this->slug ? $this->slug : Text::str_to_url($this->name);
		return true;
	}
		...
		return array(
			...
			'parent_id' => array(
				...
				'html_edit_element_param' => array(
					'call_from' => 'Categories::get_cats_selector',
				)
			),
			'name' => array(
				'name' => 'Название',
			...
			'slug' => array(
				...
				'html_edit_element' => 'extra',
				'html_edit_element_param' => array(
					'call_from' => 'CategoriesDetails::get_slug_editor_helper',
				),
			),
			'desc_short' => array(
			...
			'cat_pictures_uploader' => array(
				...
				'html_edit_element' => 'extra',
				'html_edit_element_param' => array(
					'call_from' => 'CategoriesDetails::get_picture_uploader',
				),
			),
	...
	public static function get_params_by_category($id, $group) {
		...
		$params = new Params;
		$cats_params = $params->get_list(
						array(
							'where' => ' (`subgroup` = "category" AND `object` IN (' . implode(',', $ids) . '))  OR  (`group` = "' . $group . '" AND `subgroup` = "default") '
						)
		);
	...
	public static function set_breadcrumbs($category, $path = null, $last_as_link = false) {
		...
		if (!$path) {
			Jbreadcrumbs::instance()->add($category->name);
		} else {
			unset($path[0]);
			$last = array_pop($path);
			foreach ($path as $_cat) {
				Jbreadcrumbs::instance()->add($_cat->name, joosRoute::href('category_view', array('slug' => $_cat->slug)));
				joosDocument::instance()->add_title($_cat->name);
			}
			if ($last_as_link) {
				Jbreadcrumbs::instance()->add($last->name, joosRoute::href('category_view', array('slug' => $last->slug)));
			} else {
				Jbreadcrumbs::instance()->add($last->name);
			}
...
class CategoriesDetails extends joosDBModel {
	...
	public static function get_uploader($item, $image, $i) {
		...
		if ($image_id) {
			...
			$item->name = '';
			$return .= CategoriesDetails::get_image($item, 'thumb', array('width' => '150', 'id' => $id));
		} else {
			$return .= CategoriesDetails::get_image_default(array('width' => '150', 'id' => $id));
		}
