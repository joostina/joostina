...
class actionsAjaxCoder {
	...
	public static function table_select() {
		...
		$table_fields = database::instance()->get_utils()->get_table_fields($table);
	...
	public static function generate_component($create_files = false) {
		...
		if($create_files == true){
			...
			$file = new Files;
			$file->create($c_path); $file->create($c_path. DS. 'index.html', $_blank);
			$file->create($c_path . DS . 'views'); $file->create($c_path . DS . 'views'.DS. 'index.html', $_blank);
			$file->create($c_path . DS . 'admin_views'); $file->create($c_path . DS . 'admin_views'.DS. 'index.html', $_blank);
			$file->create($c_path . DS . 'media'); $file->create($c_path . DS . 'media'.DS. 'index.html', $_blank);
		}
