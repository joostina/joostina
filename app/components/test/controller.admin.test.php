...
class actionsAdminTest {
	...
	public static function test_function() {
		$users              = joosDatabase::instance()->set_query( 'select count(id) from #__users' )->load_result();
		...
		foreach ( $reflection->getMethods() as $method ) {
			...
			if ( !empty( $matches[0] ) ) {
				foreach ( $matches[0] as $i => $row ) {
					$documentation_array['joostina_admin_menu'][$method->getName()] = array ( 'title'       => $matches[1][$i] ,
