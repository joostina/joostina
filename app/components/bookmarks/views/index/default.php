...
if(!$bookmarks){ echo 'Здесь ничего нет'; return; } 
joosLoader::lib('text');
...
<div class="box col1 b-50">
	...
	<?php $user_href = joosRoute::href('user_view', array('id' => $item->userid, 'username' => $item->username)); ?>
		<li class="m-blogs_list_item">
			<h4 class="title_item"><a class="title_item_link" href="<?php echo $view_href ?>" title="<?php echo $item->title ?>" ><?php echo $item->title ?></a></h4>
	...
	<ul class="listreset listblock listdotted">
		<?php foreach ($bookmarks['news'] as $item): ?>
			<li class="news-list_item" style="width: 45%;">				
				...
				<div class="news-list_item_date"><?php echo Bookmarks::addlink(null, array('class'=>'News','id'=>$item->id) ) ?><b>25.09</b><small>2010</small></div>
	...
	<?php  foreach ($bookmarks['video'] as $video): ?>
		<?php $video_kadr = explode('.jpg', strip_tags($video->src));
			...
			<li class="video-list_item"  style="width: 45%;">
				<a class="video-list_item_link" href="<?php echo joosRoute::href('artists_video', array('name' => $video->slug, 'id' => $video->id)) ?>">
