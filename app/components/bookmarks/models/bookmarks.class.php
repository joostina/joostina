...
class Bookmarks extends joosDBModel {
	...
	public static function by_user(User $user) {
		return database::instance()->set_query('SELECT count(id) FROM #__bookmarks WHERE user_id = ' . $user->id)->load_result();
	}

	// TODO а это зачем?
	public static function by_user_list(User $user) {
		return database::instance()->set_query('SELECT * FROM #__bookmarks WHERE user_id = ' . $user->id . ' AND obj_option!=\'games_trace\'')->load_object_list();
	}
	...
	public static function get_count(array $params = array()) {
		...
		return database::instance()->set_query($sql)->load_result();
	}
