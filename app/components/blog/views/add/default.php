	...
	<form action="/blog/add" method="post" id="blog_add" class="form_validation">
		...
		<div class="f-block">
			<div class="f f-50 f-50_1">
				...
				<input type="text" name="title" id="title" value="<?php echo $item->title ?>" class="input-100" required="required" />

			</div>
		</div>
		<?php if (User::current()->gid == 8 || User::current()->gid == 9): ?>
			<div class="f-block">
				<div class="f f-50 f-50_1">
		...
		<div class="f-block odd">
			<div class="f">
				...
				<textarea name="fulltext" id="fulltext" rows="15" cols="50" class="input-100" required="required"><?php echo $item->fulltext ?></textarea>
			</div>
		</div>
		<div class="f-block" id="form_actions">
			<div class="f f-50 f-50_1">
				<input type="hidden" name="state" value="1" />
				<input type="checkbox" name="state" value="0" id="state" <?php echo ( $item->id && $item->state == 0) ? 'checked="checked"' : '' ?> /><label  class="opt" for="state">Это черновик</label>
			</div>
