...
class actionsBlog extends joosController {
	...
	public static function action_before( $active_task ) {
		joosBreadcrumbs::instance()->add( 'Блоги' , $active_task == 'index' ? false : joosRoute::href( 'blog' ) );
	}
	...
	public static function index() {
		...
		$blog_items = $blogs->get_list( array ( 'select' => "b.*,c.slug AS cat_slug, u.gid, u.id AS userid, u.username, u.realname, comm.counter AS comments" ,
		                                        'join'   => 'AS b
		...
		joosDocument::instance()->set_page_title( 'Блоги' );

		return array ( 'blog_items' => $blog_items ,
		               'pager'      => $pager , );
	}
	...
	public static function category() {
		...
		joosBreadcrumbs::instance()->add( $blog_category->title );


		return array ( 'blog_category' => $blog_category ,
	...
	public static function view() {
		...
		// устанавливаем заголосов страницы
		joosDocument::instance()->set_page_title( $blog->title );
	...
	public static function user_blog() {
		...
		// опубликованные записи блога
		$blog_items = $blogs->get_list( array ( 'select' => "b.*,c.slug AS cat_slug, u.id AS userid, u.username AS username, comm.counter AS comments" ,
		                                        'join'   => 'AS b INNER JOIN #__blog_category AS c ON ( c.id=b.category_id AND c.state=1 )
						INNER JOIN #__users AS u ON u.id=b.user_id
						LEFT JOIN #__comments_counter AS comm ON (comm.obj_option = "Blog" AND comm.obj_id = b.id)' ,
		...
		joosBreadcrumbs::instance()->add( $user->username , joosRoute::href( 'user_view' , array ( 'id'       => $user->id ,
		                                                                                           'username' => $user->username ) ) )->add( 'Блог' );

		return array ( 'blog_items'    => $blog_items ,
		               'pager'         => $pager ,
		               'task'          => 'index' ,
		               'core::modules' => array ( 'sidebar' => array ( 'users' => array ( 'type'     => 'profile_user' ,
	...
	public static function save_blog_item( $blog ) {
		...
		if ( $blog->id ) {
			...
			joosRoute::redirect( joosRoute::href( 'blog_view' , array ( 'id'       => $blog->id ,
