...
// remove_xss
	 ...
	 */
	public static function cleanArrayXSS($ar) {
		$ret = array();

		foreach ($ar as $k => $v) {
			if (is_array($k))
				$k = self::cleanArrayXSS($k);
			else
				$k = self::remove_xss($k);

			if (is_array($v))
				$v = self::cleanArrayXSS($v);
			else
				$v = self::remove_xss($v);
	 ...
	 */
	public static function cleanXSS() {
		...
		while (list($k, $v) = each($in)) {
			foreach ($v as $key => $val) {
				...
				if (!is_array($val)) {
					...
					$val = self::cleanArrayXSS($val);
				}

				if (!is_array($key)) {
					$key = self::remove_xss($key);
				} else {
					$key = self::cleanArrayXSS($key);
				}
