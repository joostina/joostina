...
class JDBmodel extends database {
	...
	public function JDBmodel($table, $key, $db = null) {
		$this->_tbl = $table;
		$this->_tbl_key = $key;
...
// селектор выбора отношений один-ко-многим
	...
	public function find_all(array $params = array()) {
		...
		return $this->_db->setQuery(sprintf($fmtsql, implode(' AND ', $tmp)), ( isset($params['offset']) ? $params['offset'] : 0  ), ( isset($params['limit']) ? $params['limit'] : 0  ) )->loadObjectList( isset($params['key'])  ? $params['key'] : null  );
	}

}

// немного совместимости
class mosDBtable extends JDBmodel {
