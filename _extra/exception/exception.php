...
function setExceptionHandler($e) {
	...
	if (preg_match_all('/\[file\] \=\> (.+)\n/', $err, $matches)) {
		...
		if ($e instanceof PDOException) {
			foreach ($last as $k => $l) {
				if (strpos(str_replace('\\', '/', $l), str_replace('\\', '/', JPATH_SITE)) === 0) {
					$lastfile = $l;
					break;
				}
			}
...
function setErrorHandler($errno, $errstr, $errfile, $errline, $errcontext=null) {
	...
	//require
	if ($errno == 2 && strpos($errstr, 'require_once(') === 0 && strpos(str_replace('\\', '/', $errfile), str_replace('\\', '/', JPATH_BASE) . 'index.php') === 0) {
		$dmsg = debug_backtrace();
		foreach ($dmsg as $k => $a) {
			//if is user file, and must be a load method
			if (strpos(str_replace('\\', '/', $a['file']), str_replace('\\', '/', JPATH_SITE)) === 0
	...
	} else if ($errno == 2 && strpos(str_replace('\\', '/', $errfile), str_replace('\\', '/', JPATH_BASE) . 'view/DooView.php') === 0) {
		$dmsg = debug_backtrace();
		foreach ($dmsg as $k => $a) {
			if (strpos(str_replace('\\', '/', $a['file']), str_replace('\\', '/', JPATH_SITE)) === 0
	...
	if (strpos(str_replace('\\', '/', $errfile), str_replace('\\', '/', JPATH_BASE)) === 0) {
		$dmsg = debug_backtrace();
		$last = array_pop($dmsg);
		if (isset($last['file'])) {
			$errfile = $last['file'];
	...
	if (!empty($script)) {
		...
		if (strpos(str_replace('\\', '/', $errfile), str_replace('\\', '/', JPATH_BASE) . 'view') === 0) {
			$pre = '<pre>';
			$Xpre = '</pre>';
		}
	...
	$imgloader = '#index.php?doodiagnostic_pic=';

	if (ob_get_level() !== 0) {
		ob_clean();
	}
	$confData = traceVar( joosConfig::get_all() );
	...
	echo "<html><head><title>Joostina DEBUGGER - $errstr</title>";
	...
	echo '<br/><hr/><h2 style="background-color:#4370A4;color:#fff;width:320px;padding:5px;"> * Последовательность...</h2>';
	global $errTrace;
	if (!empty($errTrace)) {
		echo '<pre>';
		echo $errTrace;
...
function errorBacktrace() {
	...
	foreach ($trace as $item)
		echo '<li><span style="color:#3A66CC">' . (isset($item['file']) ? $item['file'] : '<unknown file>') . '</span><strong style="color:#DD0000">(' . (isset($item['line']) ? $item['line'] : '<unknown line>') . ')</strong> функция <span style="color:#0000BB">' . $item['function'] . '()</span></li>';
	echo '</ol>';
}
