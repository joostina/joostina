...
class jscssoptimizer {

	private static $data = array();
	private static $cacheroot;

	public static function init() {
		self::$cacheroot = JPATH_BASE . DS . 'cache' . DS . 'jcsscache' . DS;
		is_dir(self::$cacheroot) ? null : mkdir(self::$cacheroot, 0777);
		self::$data = array();
	}
	...
	public static function js(array $files) {
		...
		$cache_file = self::$cacheroot . $cache_file;
		...
		if (!is_file($cache_file)) {
			...
			foreach ($files as $file) {
				...
				self::$data[] = is_file($file) ? file_get_contents($file) : null;
			}

			$content = implode("\n", self::$data);

			include( __DIR__ . '/minifiers/js.php');

			$content = call_user_func('minify_js', $content);
		...
		return array('live' => $cache_file_live, 'file' => $cache_file);
	}
	...
	public static function css(array $files) {
		...
		$cache_file = md5(serialize($files)) . '.css';
		$cache_file = self::$cacheroot . $cache_file;
		
		if (!is_file($cache_file)) {

			foreach ($files as $file) {
				$file = str_replace(JPATH_SITE, JPATH_BASE, $file[0]);
				$file = str_replace('\\', '/', $file);
				self::$data[] = is_file($file) ? file_get_contents($file) : null;
			}
			...
			//include( __DIR__ . '/minifiers/css.php');
			//$content = call_user_func('minify_css', $content);

			//include ('/minifiers/CSSCompressor.php');
			//$content = CSSCompressor::minify($content);
			
			//include 'minifiers/gzipitm.php';
			//$content = CssMin::minify($content);
			
			//include 'minifiers/cssmin-v2.0.1.0064.php';
			//$content = CssMin::minify($content);
			
			//include 'minifiers/minify/CSS.php';
...
foreach ($files as $key => $file) {
	...
	//echo $files[$key] = $fileDir.$file.'<br />';
	echo $file . "<br />";
}
die();
if ($settings['concatenate']) {
...
if ($settings['serverCache']) {
	// Joostina code
	is_dir($settings['cacheDir']) ? null : mkdir($settings['cacheDir'], 0755, true);
