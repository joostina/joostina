...
$DB=array(
		'user'=>$db_config['user'],#required
		'pwd'=>$db_config['password'], #required
		'db'=>$db_config['name'],  #default DB, optional
		'host'=>$db_config['host'],#optional
		'port'=>$db_config['port'],#optional
...
$self = JPATH_SITE.'/administrator/index2.php?option=com_phpminiadmin&';
...
<br/>";
 ...
 <b>selected tables</b>";
	...
	$sqldr.="<table border='0' cellpadding='1' cellspacing='1' $w class='res' style='width: 100%;'>";
	$headers="<tr class='h'>";
	...
	if ($_REQUEST['d']) {
		...
		while($row=mysql_fetch_row($sth)) {
			...
			if (strlen($exsql)>$MAXI) {
				echo "INSERT INTO `$t` VALUES $exsql;$D";
				$exsql='';
			}
		}
