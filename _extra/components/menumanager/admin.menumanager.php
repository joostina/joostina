...
function showMenu($option) {
	...
	$limit = intval(joosSession::get_user_state_from_request("viewlistlimit", 'limit', $config->config_list_limit));
	$limitstart = intval(joosSession::get_user_state_from_request("view{" . $option . "}limitstart", 'limitstart', 0));
	...
	foreach ($menuTypes as $a) {
		...
		$modules = $database->set_query($query)->load_object_list();
	...
	$published = $database->set_query($query)->load_object_list();
	...
	$unpublished = $database->set_query($query)->load_object_list();
	...
	$trash = $database->set_query($query)->load_object_list();
...
function saveMenu() {
	...
	$menus = $database->set_query($query)->load_result_array();
	...
	switch ($new) {
		case 1:
			...
			$row->update_order("position=" . $database->quote($row->position));
			...
			if (!$database->set_query($query)->query()) {
		...
		default:
			$query = "SELECT id FROM #__modules WHERE module = 'mod_menu' AND params LIKE '%" . $database->get_escaped($old_menutype) . "%'";
			$modules = $database->set_query($query)->load_result_array();
...
function deleteConfirm($option, $type) {
	...
	if ($type == 'mainmenu') {
		joosFlashMessage::add(_CANNOT_RENAME_MAINMENU);
	...
	$items = $database->set_query($query)->load_object_list();

	$query = "SELECT id FROM #__modules WHERE module LIKE '%menu%' AND params LIKE '%" . $database->get_escaped($type) . "%'";
	$mods = $database->set_query($query)->load_result_array();
...
function deleteMenu($option, $cid, $type) {
	...
	if (count($mid)) {
		...
		if (!$database->set_query($query)->query()) {
			echo "<script> alert('" . $database->get_error_msg() . "');</script>\n";
			exit;
		}
	}

	joosCore::array_to_ints($cid);
	...
	if (count($cid)) {
		...
		$mod->ordering = 0;
		$mod->update_order("position='left'");
		$mod->update_order("position='right'");
	}
...
function copyMenu($option, $cid, $type) {
	...
	// copy the menu items
	$mids = joosRequest::array_param('mids');
	$total = count($mids);
	...
	foreach ($mids as $mid) {
		...
		if (!$copy->check()) {
			echo "<script> alert('" . $copy->get_error() . "'); window.history.go(-1); </script>\n";
			exit();
		}
		if (!$copy->store()) {
			echo "<script> alert('" . $copy->get_error() . "'); window.history.go(-1); </script>\n";
