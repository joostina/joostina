...
//Деревооо
mosMainFrame::addLib('tree', 'includes/libraries/joomlatune');
...
?>
	...
	<div class="comments-list" id="comments-list-0">
		...
		foreach ($items as $id => $comment) :			
			...
			<?php if ($comment->children == 0) { echo '</div>'; } ?>
			<?php if ($comment->level > 0 && $i == $count - 1) {
				$j = $comment->level;
			} ?>
			<?php while ($j > 0) { ?>
			</div><!--//Comment item-->			
			...
			<?php $i++; $currentLevel = $comment->level; ?>
